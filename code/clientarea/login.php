...
if(isset($_POST['submit'])){
    ...
    if(!isset($acc[$indexid]['客戶編號'])){
        echo "<script>alert('帳號或密碼錯誤');window.location.href = './login';</script>";
        exit();
    }
    if (!password_verify($password, $acc[$indexid]['顧客密碼'])) {
        echo "<script>alert('帳號或密碼錯誤');window.location.href = './login';</script>";
        exit();
    } 
    setcookie("uid", $acc[$indexid]['客戶編號'], time() + (86400 * 30), "/");
    setcookie("name", $acc[$indexid]['客戶名稱'], time() + (86400 * 30), "/");
                        ...
                        <div class="row justify-content-center">
                            <div class="col-lg-5">
                                <div class="card shadow-lg border-0 rounded-lg mt-5">
                                    <div class="card-header"><h3 class="text-center font-weight-light my-4">樂創後台 - 登入</h3></div>
                                    <div class="card-body">
