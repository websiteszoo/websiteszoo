...
function showModal( modalElementSelector, modalContentData, isFirstModal = false ) {
	...
	const modalActionBtn = modalNode.find( '.modal-footer .scc-setup-wizard-button' );
...
function handleQuizBtnClick( evt ) {
	...
	if ( currentStep == 0 ) {
		...
		const isEmailOptInEnabled = document.querySelector( '#wq_optin_for_comm' ).checked;
		...
		const resultsEmailFormData = {
			optin: document.querySelector( '#wq_optin_for_comm' ).checked,
	...
	if ( isNaN( currentStep ) ) {
		...
		showModal( 'quizResult', {
			title: 'Your Customized Cost Calculator Recommendations',
			subtitle: `Fill out the form to receive instructions by Email`,
			modalLead: '',
			currentStep,
...
function handleEmailOptInForWQ( $this ) {
	const shownModalActionBtn = document.querySelector( '.modal.show .modal-footer .scc-setup-wizard-button' );
	if ( $this.checked ) {
