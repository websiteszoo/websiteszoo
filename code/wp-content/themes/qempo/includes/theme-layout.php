...
================================================== */
if ( ! function_exists( 'qempo_base_sidebar' ) ) {
    function qempo_base_sidebar() {
        // VARIABLES
        global $post;

        // DEFAULT SIDEBAR CONFIG
        $default_sidebar_config = qempo_get_option('default_sidebar_config', ''); 
        $default_left_sidebar   = qempo_get_option('default_left_sidebar', ''); 
        $default_right_sidebar  = qempo_get_option('default_right_sidebar', '');
        ...
        if ( is_search() || is_archive() || is_author() || is_category() || is_home() ) {
          $default_sidebar_config = qempo_get_option('archive_sidebar_config', ''); 
          $default_left_sidebar   = qempo_get_option('archive_sidebar_left', '');
          $default_right_sidebar  = qempo_get_option('archive_sidebar_right', ''); 
        }
        ...
        if ( $post && is_singular() ) {
          $sidebar_config = get_post_meta( $post->ID, 'qempo_sidebar_config', true );
          $left_sidebar   = get_post_meta( $post->ID, 'qempo_left_sidebar', true );
          $right_sidebar  = get_post_meta( $post->ID, 'qempo_right_sidebar', true );
        }
...
function qempo_sidebar_global($sidebar_layout_config, $sidebar_left_config, $sidebar_right_config){
...
function qempo_base_layout($template){
  $post_id = qempo_id();
  $sidebar_var           = qempo_base_sidebar();
  ...
  $sidebar_config = qempo_sidebar_global($sidebar_var['config'], $sidebar_var['left'], $sidebar_var['right']);
  extract($sidebar_config);
  
  $page_full_width = get_post_meta($post_id, 'qempo_page_full_width', true);
  ...
  <?php 
    $class_container = $page_full_width ? 'container-layout-content container-full' : 'container-layout-content container';
  ?>
    <div class="<?php echo esc_attr($class_container); ?>">
    <div class="content-page-wrap">
      <?php do_action( 'qempo_before_main_content' ); ?>
       <div class="main-page-content base-layout row <?php echo esc_attr($page_wrap_class); ?>">
          
            <div class="content-page <?php echo esc_attr($main_content_config['class']); ?>">
              <div class="content-page-inner">   
                  <?php qempo_get_template($template) ?>
              </div>    
            </div>      
        ...
        <?php do_action( 'qempo_after_main_content' ); ?>
      </div>
    </div>
...
if ( ! function_exists( 'qempo_get_template' ) ) {
    function qempo_get_template( $template ) {
